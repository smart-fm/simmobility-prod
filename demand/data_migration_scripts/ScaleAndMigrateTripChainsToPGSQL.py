...
######### Update this section to insert into the corresponding tables ##########
tbl_activities = "Preday_Scaled_Activities"
tbl_trips = "Preday_Scaled_Trips"
tbl_sub_trips = "Preday_Scaled_Sub_Trips"
tbl_trip_chains = "Preday_Scaled_Trip_Chain"
###############################################################################
...
def insert_activity(a, appendage = ""):
	query = "INSERT INTO \"" + tbl_activities + "\" VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);"
	data = (a.Activity_Id+appendage, a.Activity_Type, a.Primary_Activity, a.Flexible_Activity, a.Location_Id, a.Location_Type, a.Activity_Start_Time, a.Activity_End_Time, a.Mandatory_Activity)
	pg_cursor.execute(query, data)
	print "inserted into" + tbl_activities

def insert_sub_trip(st, appendage = ""):
	query = "INSERT INTO \"" + tbl_sub_trips + "\" VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);"
	data = (st.TripId+appendage, st.Sub_Trip_Id+appendage, st.FromLocationId, st.FromLocationType, st.ToLocationId, st.ToLocationType, st.Mode, st.Primary_Mode_Of_Travel, st.Start_Time, st.sequence_num)
	pg_cursor.execute(query, data)
	print "inserted into" + tbl_sub_trips

def insert_trip(t, appendage = ""):
	query = "INSERT INTO \"" + tbl_trips + "\" VALUES (%s, %s, %s, %s, %s);"
	data = (t.Trip_Id+appendage, t.FromLocationId, t.FromLocationType, t.ToLocationId, t.ToLocationType)
	pg_cursor.execute(query, data)
	print "inserted into" + tbl_trips
	for st in t.sub_trips:
		insert_sub_trip(st, appendage)

def insert_trip_chain(tc, pid, scale = 1):
	for k in range(scale):
		appendage = "_" + str(k)
		query = "INSERT INTO \"" + tbl_trip_chains + "\" VALUES (%s, %s, %s, %s, %s);"
		data = None
		for item in tc:
			if isinstance(item, Activity):
				insert_activity(item, appendage)
				data = (pid+appendage, item.sequence_num, "Activity", 0, item.Activity_Id+appendage)
			elif isinstance(item, Trip):
				insert_trip(item, appendage)
				data = (pid+appendage, item.sequence_num, "Trip", item.Trip_Id+appendage, 0)
			pg_cursor.execute(query, data)
			print "inserted into" + tbl_trip_chains
		conn.commit()	
		print "Committed"
...
#	for person_id in person_ids_test:
	for person_id in person_ids:
		...
		hh_data = db[collection_names["HITS2008_RAW"]].find_one({"h1_hhid" : p_data['hhid']})
		scale = int(hh_data["hhfactors_200409"])
		insert_trip_chain(trip_chain, pid, scale)
