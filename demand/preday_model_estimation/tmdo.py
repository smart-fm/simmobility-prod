...
#beta_cost_mrt= Beta('MRT cost',0,-10,10,0)
beta_cost_private_bus_2 =Beta('private bus cost',0,-10,10,1)
beta_cost_drive1_2= Beta('drive1 cost',0,-10,10,1)
beta_cost_share2_2= Beta('shared ride 2 cost',0,-10,10,1)
beta_cost_share3_2= Beta('shared ride 3 cost',0,-10,10,1)
beta_cost_motor_2 = Beta('motor cost',0,-10,10,1)
beta_cost_taxi_2 = Beta('taxi cost',0,-10,10,1)
...
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_drive1+cost_over_income_private_bus_%s * (1-missing_income) * beta_cost_private_bus_1 + cost_private_bus_%s*beta_cost_drive1_2 + tt_private_bus_%s * beta_tt_private_bus+beta_central_private_bus * central_dummy_%s + beta_log * log(exp(beta_area)*area_%s+exp(beta_population)*population_%s) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_private_bus + beta_female_private_bus * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j)) 
...
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_drive1+cost_over_income_taxi_%s *(1-missing_income)* beta_cost_taxi_1 + cost_taxi_%s*beta_cost_drive1_2 + tt_taxi_%s * beta_tt_taxi+beta_central_taxi * central_dummy_%s + beta_log * log(exp(beta_area)*area_%s+exp(beta_population)*population_%s)+ (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_taxi + beta_female_taxi * Female_dummy" % (V_counter,j,j,j,j,j,j,j,j)) 
