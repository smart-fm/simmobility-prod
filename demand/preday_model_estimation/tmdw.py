...
beta_population = Beta('beta for population',0,-1000,10,1)
...
for j in range(1,1093):
	V_counter=V_counter+1
	exec("V_%s = beta_cons_walk+tt_walk_%s*beta_tt_walk +beta_central_walk * central_dummy_%s+ beta_log * log(employment_%s+exp(beta_area)*area_%s+(beta_population)*population_%s+1) + (walk_distance_first_%s+walk_distance_second_%s)*beta_distance_walk + beta_female_walk * Female_dummy" % (V_counter,j,j,j,j,j,j,j))
