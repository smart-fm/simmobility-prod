...
for count,comb in enumerate(combs):
    ...
    V[count+1]=eval("sarr_1(%s) + sdep_1(%s) + gender * (sarr_2(%s)+sdep_2(%s)) + DUR_1 * %s + DUR_2 * %s**2 + DUR_3 * %s**3 " % (arr,dep,arr,dep,dur,dur,dur) + \
...
exclude = (( PrimaryActivityIndex == 1)+( PrimaryActivityIndex == 2)+(PrimaryActivityIndex == 3) + (AggregateModeIndex_HT1==2)*(ArrivalPeriodIndex<5) + (AggregateModeIndex_HT2==2)*(DeparturePeriodIndex>46))>0 
