	...
	//	checkForCollisions();
		...
		if(isGoalReached()){
			...
			if(currentStage==0||currentStage==2){
				...
				{
					boost::mutex::scoped_lock local_lock(BufferedBase::global_mutex);
					BufferedBase::log_file() <<"("<<"\"pedestrian\","<<frameNumber<<","<<parent->getId()<<","<<"{\"xPos\":\""<<parent->xPos.get()*100<<"\"," <<"\"yPos\":\""<<this->parent->yPos.get()*100<<"\","<<"\"pedSig\":\""<<currPhase<<"\",})"<<std::endl;
				}
			}
