...
void sim_mob::Pedestrian::setGoal(int stage) //0-to the next intersection, 1-to the crossing end, 2-to the destination
{
	...
	if (stage == 0) {
		...
		setSidewalkParas(ConfigParams::GetInstance().getNetwork().locateNode(Point2D(37250760, 14355120), true),
				parent->destNode, true);
...
void sim_mob::Pedestrian::setCrossingParas() {
	...
	std::set<sim_mob::RoadSegment*>::const_iterator i;
	const MultiNode* currNode = dynamic_cast<const MultiNode*> (ConfigParams::GetInstance().getNetwork().locateNode(
			goal, true));
	...
	for (i = roadsegments.begin(); i != roadsegments.end(); i++) {
		if ((*i)->getLink()->getStart() != parent->originNode && (*i)->getLink()->getEnd() != parent->originNode
				&& (*i)->getLink()->getStart() != parent->destNode && (*i)->getLink()->getEnd() != parent->destNode) {
