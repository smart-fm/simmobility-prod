...
void EstablishmentDao::fromRow(Row& result, Establishment& outObj)
{
    ...
    outObj.size = result.get<BigSerial>("size", INVALID_ID);;
    outObj.revenue = result.get<BigSerial>("revenue", INVALID_ID);;
    outObj.grossSqM = result.get<BigSerial>("gross_sq_m", INVALID_ID);;
