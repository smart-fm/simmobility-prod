...
Household::Household( BigSerial id, BigSerial lifestyleId, BigSerial unitId, BigSerial ethnicityId, BigSerial vehicleCategoryId,  int size, int childUnder4, int childUnder15, int adult, double income,
					  ...
					  int buySellInterval, std::tm moveInDate,int timeOnMarket,int timeOffMarket,int isBidder,int isSeller,int hasMoved, int tenureStatus, int awakenedDay, bool existInDB, std::tm lastAwakenedDate,int lastBidStatus): id(id), lifestyleId(lifestyleId), unitId(unitId),
					  ...
					  moveInDate(moveInDate),  timeOnMarket(timeOnMarket),timeOffMarket(timeOffMarket),isBidder(isBidder),isSeller(isSeller),hasMoved(hasMoved), tenureStatus(tenureStatus), awakenedDay(awakenedDay), existInDB(existInDB), lastAwakenedDate(lastAwakenedDate),lastBidStatus(lastBidStatus){}
...
Household::Household(): id(0), lifestyleId(0), unitId(0), ethnicityId(0), vehicleCategoryId(0),size(0), childUnder4(0), childUnder15(0), adult(0),income(0), housingDuration(0), workers(0), ageOfHead(0),
						...
						isSeller(0),hasMoved(0), tenureStatus(0), awakenedDay(0),existInDB(false), lastAwakenedDate(std::tm()),lastBidStatus(0){}
...
std::tm Household::getLastAwakenedDate()
{
	return lastAwakenedDate;
}

void Household::setLastAwakenedDate(std::tm lastAwkenedDate)
{
	this->lastAwakenedDate = lastAwkenedDate;
}
