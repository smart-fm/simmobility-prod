...
Individual::Individual(BigSerial id, BigSerial individualTypeId, BigSerial householdId,	BigSerial jobId, BigSerial ethnicityId,	BigSerial employmentStatusId, BigSerial genderId,
					   BigSerial educationId, BigSerial occupationId, BigSerial vehiculeCategoryId,	BigSerial transitCategoryId, BigSerial ageCategoryId, BigSerial residentialStatusId,
					   bool householdHead, float income, int memberId, bool workerAtHome,bool driversLicence, std::tm dateOfBirth )
					   ...
					   ageCategoryId(ageCategoryId), residentialStatusId(residentialStatusId), householdHead(householdHead), income(income), memberId(memberId), workerAtHome(workerAtHome),
...
Individual& Individual::operator=( const Individual& source)
{
	...
	this->workerAtHome = source.workerAtHome;
...
bool Individual::getWorkerAtHome() const
{
	return workerAtHome;
}

std::tm Individual::getDriversLicence() const
{
	...
	namespace long_term
	{
		std::ostream& operator<<(std::ostream& strm, const Individual& data)
		{
			return strm << "{"
						...
						<< "\"vehiculeCategoryId \":\"" << data.vehiculeCategoryId << "\","
						...
						<< "\"workerAtHome \":\"" << data.workerAtHome << "\","
