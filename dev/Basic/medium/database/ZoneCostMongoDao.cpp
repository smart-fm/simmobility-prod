...
bool sim_mob::medium::ZoneMongoDao::getAllZones(boost::unordered_map<int, ZoneParams*>& outList)
{
...
void sim_mob::medium::ZoneMongoDao::fromRow(mongo::BSONObj document, ZoneParams& outParam)
{
	...
	if (document.hasElement(MONGO_FIELD_ZONE_CBD_ZONE))
	{
		outParam.setCbdDummy(document.getField(MONGO_FIELD_ZONE_CBD_ZONE).Number());
	}
	else
	{
		outParam.setCbdDummy(false);
	}
}
