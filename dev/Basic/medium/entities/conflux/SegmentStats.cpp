 ...
 */
SupplyParams::SupplyParams(const RoadSegment* rdSeg, double statsLength) :
		freeFlowSpeed(convertKmphToCmps(rdSeg->maxSpeed)), minSpeed(0.2 * freeFlowSpeed), /*20% of free flow speed as suggested by Yang Lu*/
		jamDensity(0.2), /*density during traffic jam in veh/meter*/
...
std::string SegmentStats::reportSegmentStats(uint32_t frameNumber)
{
	...
	if (ConfigManager::GetInstance().CMakeConfig().OutputEnabled())
	{
		...
		msg << "(\"segmentState\"" << "," << frameNumber << "," << roadSegment << ",{" << "\"speed\":\"" << segVehicleSpeed << "\",\"flow\":\"" << segFlow
				...
				<< "\",\"segment_length\":\"" << length << "\",\"segment_id\":\"" << roadSegment->getSegmentAimsunId() << "\",\"stats_num\":\""
...
void SegmentStats::printBusStops() const
{
	...
	printStream << "Segment: " << roadSegment->getSegmentAimsunId() << "|link: " << roadSegment->getLink()->getLinkId() << "|stats#: " << statsNumberInSegment
