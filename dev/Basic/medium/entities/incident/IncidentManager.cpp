...
//		throw std::runtime_error(out.str());
	...
	while (getline(in,line))
	{
		...
		sim_mob::Logger::instance["path_set"] << "Incident inserted for tick:" <<tick << " sectionId:" << sectionId << sim_mob::Logger::newLine;
	}
	in.close();
}
...
void sim_mob::IncidentManager::insertTickIncidents(uint32_t tick){
	...
	for(std::multimap<uint32_t,Incident>::iterator incident = tickIncident.first; incident != tickIncident.second; incident++){
		...
		sim_mob::Logger::instance["path_set"] << " INCIDENT  segment:"<< rs->getSegmentAimsunId() << " affected:" << persons.size() << sim_mob::Logger::newLine;
//		/**
//		 * DEBUG
//		 */
	...
	for(RPOD::const_iterator it(range.first); it != range.second; it++){
		...
		for(itSS = path.begin(); (*itSS) != curSS ; itSS++){
			if(targetRS == (*itSS)->getRoadSegment()){
				res = true;
//				sim_mob::Logger::instance["path_set"] << "This incident is happening on a segments 'before' this driver" << std::endl;
				break;
			}
		}
...
//			sim_mob::Logger::instance["path_set"] << "This incident is happening on the driver's current segment" << std::endl;
			res = true;
		}

		if(res){
			ignored++;
//			sim_mob::Logger::instance["path_set"] << "ignoring this driver" <<std::endl;
	...
	sim_mob::Logger::instance["path_set"] << "Number of Affected Driver's Paths: " << affected << std::endl;
	sim_mob::Logger::instance["path_set"] << "Number of Affected Drivers: " << affected - ignored << std::endl;
	sim_mob::Logger::instance["path_set"] << "Number of Drivers Reacting to Incident: " << reacting << std::endl;
	sim_mob::Logger::instance["path_set"] << "Number of Drivers Ignoring the incident : " << ignorant << std::endl;
}
