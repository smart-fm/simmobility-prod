...
//		throw std::runtime_error(out.str());
	...
	while (getline(in,line))
	{
		...
		sim_mob::Profiler::instance["path_set"] << "Incident inserted for tick:" <<tick << " sectionId:" << sectionId << sim_mob::Profiler::newLine;
	}
	in.close();
}
...
void sim_mob::IncidentManager::insertTickIncidents(uint32_t tick){
	...
	for(std::multimap<uint32_t,Incident>::iterator incident = tickIncident.first; incident != tickIncident.second; incident++){
		...
		sim_mob::Profiler::instance["path_set"] << " INCIDENT  segment:"<< rs->getSegmentAimsunId() << " affected:" << persons.size() << sim_mob::Profiler::newLine;
//		/**
//		 * DEBUG
//		 */
