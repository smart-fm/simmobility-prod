...
namespace sim_mob
{
	namespace medium
	{
		...
		void TaxiDriverMovement::frame_init()
		{
			...
			parentTaxiDriver->setDriveMode(TaxiDriver::CRUISE);
			assignFirstNode();
		...
		const Lane* TaxiDriverMovement::getBestTargetLane(const SegmentStats* nextSegStats, const SegmentStats* nextToNextSegStats)
		{
			...
			TaxiDriver::DriverMode mode = parentTaxiDriver->getDriverMode();
			...
			else
			{
				...
				for (std::vector<Lane* >::const_iterator lnIt=lanes.begin(); lnIt!=lanes.end(); ++lnIt)
				{
					...
					if (!lane->isPedestrianLane())
					{
						...
						{
							if(parentTaxiDriver->getDriverMode()!=TaxiDriver::CRUISE)
							{
						...
						{
							if(nextToNextSegStats||parentTaxiDriver->getDriverMode() != TaxiDriver::CRUISE)
							{
								continue;
							}
						}
		...
		bool TaxiDriverMovement::moveToNextSegment(DriverUpdateParams& params)
		{
			...
			if((curRs->getParentLink()->getLinkId() ==1||curRs->getParentLink()->getLinkId() == 6000)&&boost::iequals(parentTaxiDriver->getParent()->getDatabaseId(),"Taxi2"))
			{
				int debug =1;
			}
			...
			{
				//vehicle is done
				std::string id = parentTaxiDriver->getParent()->getDatabaseId();
				pathMover.advanceInPath();
			...
			if (isNewLinkNext||isEndOfLink_CruiseMode)
			{
				...
				if(isEndOfLink_CruiseMode)
				{
					...
					parentTaxiDriver->checkPersonsAndPickUpAtNode(params.now);
		...
		void TaxiDriverMovement::selectNextNodeAndLinksWhileCruising()
		{
			...
			if(boost::iequals(parentTaxiDriver->getParent()->getDatabaseId(),"Taxi5"))
			{
				int debug =1 ;
			}
			if(!currentNode)
			...
			if(nodeVector.size()>0)
			{
				...
				if(selectedLink->getLinkId()==6000)
				{
					int debug =1;
				}
				destinationNode = selectedLink->getToNode();
				currentRoute.erase(currentRoute.begin(),currentRoute.end());
				currentRoute.insert(currentRoute.end(),selectedLink->getRoadSegments_1().begin(),selectedLink->getRoadSegments_1().end());
		...
		{
			parentTaxiDriver->setDriveMode(TaxiDriver::DRIVE_TO_TAXISTAND);
			SubTrip currSubTrip;
