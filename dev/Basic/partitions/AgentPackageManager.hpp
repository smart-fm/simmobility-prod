...
void sim_mob::AgentPackageManager::packageOneCrossDriver(Archive & ar, Agent const* agent)
{
	...
	Driver *one_driver = const_cast<Driver*> (driver);
	...
	rnpackageImpl.packageFixedDelayedPointer2D(ar, one_driver->perceivedVelocity);
	rnpackageImpl.packageFixedDelayedPointer2D(ar, one_driver->perceivedVelocityOfFwdCar);
	rnpackageImpl.packageFixedDelayedInt(ar, one_driver->perceivedDistToFwdCar);
