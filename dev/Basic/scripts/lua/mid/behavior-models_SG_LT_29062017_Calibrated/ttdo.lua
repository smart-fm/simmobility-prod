...
local function computeUtilities(params,dbparams) 
	...
	for i = 1,1176 do
		...
		--print(" Utility for option ",i," = ",utility[i]," mode ",dbparams.mode," arr ",arr," sarr_1 ",sarr_1(arr)," sarr_2 ",sarr_2(arr)," dep ",dep, " sdep_1 ",sdep_1(dep)," sdep_2(dep) ",sdep_2(dep)," gender ",gender," Time Interval ", arrmidpoint[comb[i][1]]," ",depmidpoint[comb[i][2]]," TT1 ",dbparams:TT_HT2(depid)," TT2 ",dbparams:TT_HT1(arrid), " costHT1_am ",cost_HT1_am," cost_HT1_pm ",cost_HT1_pm, " cost_HT1_op ",cost_HT1_op," cost_HT2_am ",cost_HT2_am," cost_HT2_pm ",cost_HT2_pm," cost_HT2_op ",cost_HT2_op)        
...
local function computeAvailabilities(params,dbparams)
	...
	-- print(" PersonID: ",params.person_id)
	for i = 1, 1176 do 
		availability[i] = params:getTimeWindowAvailabilityTour(i,mode)
		sum_avail = sum_avail + availability[i]
		-- print(" Utility for option ",i," = ",utility[i]," Availability ",availability[i]," Time Window ",comb[i]," Time Interval ", arrmidpoint[comb[i][1]]," ",depmidpoint[comb[i][2]])        
	end
end
...
function choose_ttdo(params,dbparams)
	...
	local temp_choice = make_final_choice(probability)
	if temp_choice == 1176 then
		print("PersonID: ",params.person_id," TTDO Choice: ",temp_choice, " Sum_Avail ",sum_avail," Interval ",arrmidpoint[comb[temp_choice][1]]," ",arrmidpoint[comb[temp_choice][2]])
	end
	return temp_choice
