...
local beta_cost_Rail_AMOD_1 = -0.19
local beta_cost_Rail_AMOD_Pool_1 = -0.19
...
local beta_tt_Rail_AMOD = -3.78
local beta_tt_Rail_AMOD_Pool = -3.78
local beta_tt_AMOD = 0
local beta_tt_AMOD_Pool = 0
...
local beta_central_Rail_AMOD = 0
local beta_central_Rail_AMOD_Pool = 0
local beta_central_AMOD = 0
local beta_central_AMOD_Pool = 0
...
local beta_cons_Rail_SMS = -4.528
local beta_cons_Rail_SMS_Pool = -11.955
local beta_cons_private_bus = -3.462
local beta_cons_drive1 = 0
local beta_cons_share2 = -3.944
local beta_cons_share3 = -5.575
local beta_cons_motor = -10.008
local beta_cons_walk = -8.024
local beta_cons_taxi = -9.675
local beta_cons_SMS = -10.090
local beta_cons_SMS_Pool = -12.672
local beta_cons_Rail_AMOD = -4.528
local beta_cons_Rail_AMOD_Pool = -11.955
local beta_cons_AMOD = -10.090
local beta_cons_AMOD_Pool = -12.672
...
local beta_female_Rail_AMOD = 0
local beta_female_Rail_AMOD_Pool = 0
...
local beta_mode_work_Rail_AMOD = 0
local beta_mode_work_Rail_AMOD_Pool = 0
...
local function computeUtilities(params,dbparams)
	...
	local mode_work_Rail_AMOD = dbparams.mode_to_work == 2 and 1 or 0
	local mode_work_Rail_AMOD_Pool = dbparams.mode_to_work == 2 and 1 or 0
	...
	for i =1,1169 do
		...
		cost_SMS[i] = (cost_SMS_1[i] + cost_SMS_2[i])*0.6
		...
		cost_SMS_Pool[i] = (cost_SMS_Pool_1[i] + cost_SMS_Pool_2[i])*0.6*0.7
		...
		cost_AMOD[i] = (cost_AMOD_1[i] + cost_AMOD_2[i])*0.6/3
		...
		cost_AMOD_Pool[i] = (cost_AMOD_Pool_1[i] + cost_AMOD_Pool_2[i])*0.6*0.7/3
		...
		cost_Rail_SMS_Pool[i] = cost_mrt[i] + cost_Rail_SMS_AE_Pool_avg[i]*0.6*0.7
		...
		cost_Rail_AMOD[i] = cost_mrt[i] + cost_Rail_AMOD_AE_avg[i]*0.6/3
		...
		cost_Rail_AMOD_Pool[i] = cost_mrt[i] + cost_Rail_AMOD_AE_Pool_avg[i]*0.6*0.7/3
		...
		tt_Rail_AMOD_Pool[i] = tt_public_ivt[i]+ tt_public_out[i]/6.0+(aed+aed)/60+1/10	
		tt_AMOD[i] = tt_car_ivt[i] + 1.0/6
		tt_AMOD_Pool[i] = tt_car_ivt[i] + 1.0/6+ 1/10+(d1[i]+d2[i])/2/60 + 1.0/6
	...
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = 0 -- beta_cons_AMOD + cost_AMOD[i] * beta_cost_drive1_1 + tt_AMOD[i] * beta_tt_AMOD + beta_central_AMOD * central_dummy[i] + beta_log * log(shop[i]+exp(beta_employment)*employment[i]) + (d1[i]+d2[i]) * beta_distance_AMOD + beta_female_AMOD * female_dummy
	end
	
	--utility function for Rail_AMOD 1-1169
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = 0 -- beta_cons_Rail_AMOD + cost_Rail_AMOD[i] * beta_cost_Rail_AMOD_1 + tt_Rail_AMOD[i] * beta_tt_Rail_AMOD + beta_central_Rail_AMOD * central_dummy[i] + beta_log * log(shop[i]+exp(beta_employment)*employment[i]) + (d1[i]+d2[i]) * beta_distance_Rail_AMOD + beta_female_Rail_AMOD * female_dummy + beta_mode_work_bus * mode_work_Rail_AMOD
	end
		
	--utility function for AMOD_Pool 1-1169
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = 0 -- beta_cons_AMOD_Pool + cost_AMOD_Pool[i] * beta_cost_drive1_1 + tt_AMOD_Pool[i] * beta_tt_AMOD_Pool + beta_central_AMOD_Pool * central_dummy[i] + beta_log * log(shop[i]+exp(beta_employment)*employment[i]) + (d1[i]+d2[i]) * beta_distance_AMOD_Pool + beta_female_AMOD_Pool * female_dummy
	end
	
	--utility function for Rail_AMOD_Pool 1-1169
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = 0 --beta_cons_Rail_AMOD_Pool + cost_Rail_AMOD_Pool[i] * beta_cost_Rail_AMOD_Pool_1 + tt_Rail_SMS_Pool[i] * beta_tt_Rail_AMOD_Pool + beta_central_Rail_AMOD_Pool * central_dummy[i] + beta_log * log(shop[i]+exp(beta_employment)*employment[i]) + (d1[i]+d2[i]) * beta_distance_Rail_AMOD_Pool + beta_female_Rail_AMOD_Pool * female_dummy + beta_mode_work_bus * mode_work_Rail_AMOD_Pool
	end
end
