...
local beta_cost_Rail_AMOD_2 = -0.438
local beta_cost_Rail_AMOD_Pool_2 = -0.438
...
local beta_tt_Rail_AMOD = -3.75
local beta_tt_Rail_AMOD_Pool = -3.75
local beta_tt_AMOD_first = -5.63
local beta_tt_AMOD_second = -5.79
local beta_tt_AMOD_Pool_first = -5.63
local beta_tt_AMOD_Pool_second = -5.79
...
local beta_central_Rail_AMOD = 0.172
local beta_central_Rail_AMOD_Pool = 0.172
local beta_central_AMOD = 1.07
local beta_central_AMOD_Pool = 1.07
...
local beta_distance_Rail_AMOD = 0.0112
local beta_distance_Rail_AMOD_Pool = 0.0112
local beta_distance_AMOD = 0.000622
local beta_distance_AMOD_Pool = 0.000622

local beta_cons_bus = 4.479
local beta_cons_mrt = 4.383
local beta_cons_Rail_SMS = 3.170
local beta_cons_Rail_SMS_Pool = -4.443
...
local beta_cons_taxi = -5.728
local beta_cons_SMS = -6.143
local beta_cons_SMS_Pool = -7.739
local beta_cons_Rail_AMOD = 3.170
local beta_cons_Rail_AMOD_Pool = -4.443
local beta_cons_AMOD = -6.143
local beta_cons_AMOD_Pool = -7.739
...
local beta_female_Rail_AMOD = 0.622
local beta_female_Rail_AMOD_Pool = 0.622
local beta_female_AMOD = 1.40
local beta_female_AMOD_Pool = 1.40
...
local function computeUtilities(params,dbparams)
	...
	for i =1,1169 do
		...
		cost_SMS_Pool[i] = (cost_SMS_Pool_1[i] + cost_SMS_Pool_2[i])*0.6*0.7/2
		
		cost_AMOD_1[i] = 3.4+((d1[i]*(d1[i]>10 and 1 or 0)-10*(d1[i]>10 and 1 or 0))/0.35+(d1[i]*(d1[i]<=10 and 1 or 0)+10*(d1[i]>10 and 1 or 0))/0.4)*0.22+ dbparams:cost_car_ERP(i) + central_dummy[i]*3
		cost_AMOD_2[i] = 3.4+((d2[i]*(d2[i]>10 and 1 or 0)-10*(d2[i]>10 and 1 or 0))/0.35+(d2[i]*(d2[i]<=10 and 1 or 0)+10*(d2[i]>10 and 1 or 0))/0.4)*0.22+ central_dummy[i]*3
		cost_AMOD[i] = (cost_AMOD_1[i] + cost_AMOD_2[i])*0.6/2/3

		cost_AMOD_Pool_1[i] = 3.4+((d1[i]*(d1[i]>10 and 1 or 0)-10*(d1[i]>10 and 1 or 0))/0.35+(d1[i]*(d1[i]<=10 and 1 or 0)+10*(d1[i]>10 and 1 or 0))/0.4)*0.22+ dbparams:cost_car_ERP(i) + central_dummy[i]*3
		cost_AMOD_Pool_2[i] = 3.4+((d2[i]*(d2[i]>10 and 1 or 0)-10*(d2[i]>10 and 1 or 0))/0.35+(d2[i]*(d2[i]<=10 and 1 or 0)+10*(d2[i]>10 and 1 or 0))/0.4)*0.22+ central_dummy[i]*3
		cost_AMOD_Pool[i] = (cost_AMOD_Pool_1[i] + cost_AMOD_Pool_2[i])*0.6*0.7/2/3
		...
		cost_Rail_SMS_Pool[i] = cost_public[i] + cost_Rail_SMS_AE_Pool_avg[i]*0.6*0.7
		
		cost_Rail_AMOD_AE_1[i] =     3.4+((aed*(aed>10 and 1 or 0)-10*(aed>10 and 1 or 0))/0.35+(aed*(aed<=10 and 1 or 0)+10*(aed>10 and 1 or 0))/0.4)*0.22+ dbparams:cost_car_ERP(i) + central_dummy[i]*3
		cost_Rail_AMOD_AE_2[i] = 3.4+((aed*(aed>10 and 1 or 0)-10*(aed>10 and 1 or 0))/0.35+(aed*(aed<=10 and 1 or 0)+10*(aed>10 and 1 or 0))/0.4)*0.22+ central_dummy[i]*3
		cost_Rail_AMOD_AE_avg[i] = (cost_Rail_AMOD_AE_1[i] + cost_Rail_AMOD_AE_2[i])/2

		cost_Rail_AMOD[i] = cost_public[i] + cost_Rail_AMOD_AE_avg[i]*0.6/3
		
		local aed = 2.0 -- Access egress distance
		cost_Rail_AMOD_AE_Pool_1[i] = 3.4+((aed*(aed>10 and 1 or 0)-10*(aed>10 and 1 or 0))/0.35+(aed*(aed<=10 and 1 or 0)+10*(aed>10 and 1 or 0))/0.4)*0.22+ dbparams:cost_car_ERP(i) + central_dummy[i]*3
		cost_Rail_AMOD_AE_Pool_2[i] = 3.4+((aed*(aed>10 and 1 or 0)-10*(aed>10 and 1 or 0))/0.35+(aed*(aed<=10 and 1 or 0)+10*(aed>10 and 1 or 0))/0.4)*0.22+ central_dummy[i]*3
		cost_Rail_AMOD_AE_Pool_avg[i] = (cost_Rail_AMOD_AE_Pool_1[i] + cost_Rail_AMOD_AE_Pool_2[i])/2


		cost_Rail_AMOD_Pool[i] = cost_public[i] + cost_Rail_AMOD_AE_Pool_avg[i]*0.6*0.7/3
		...
		tt_Rail_AMOD[i] = tt_public_ivt[i]+ tt_public_out[i]/6.0
		tt_Rail_AMOD_Pool[i] = tt_public_ivt[i]+ tt_public_out[i]/6.0+(aed+aed)/60/2+1/20
		tt_AMOD[i] = tt_car_ivt[i] + 1.0/12
		tt_AMOD_Pool[i] = tt_car_ivt[i] + 1.0/12 + 1/20+(d1[i]+d2[i])/4/60
	...
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_AMOD + first_bound * cost_AMOD[i]* beta_cost_drive1_2_first + second_bound * cost_AMOD[i]* beta_cost_drive1_2_second + first_bound * tt_AMOD[i] * beta_tt_AMOD_first + second_bound * tt_AMOD[i] * beta_tt_AMOD_second + beta_central_AMOD * central_dummy[i] + beta_shop * log(1+shop[i])*shop_stop_dummy+beta_work * log(1+employment[i])*work_stop_dummy + (d1[i]+d2[i]) * beta_distance_AMOD + beta_female_AMOD * female_dummy
	end
	--utility function for Rail_AMOD 1-1169
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_Rail_AMOD + cost_Rail_AMOD[i] * beta_cost_Rail_AMOD_2 + tt_Rail_AMOD[i] * beta_tt_Rail_AMOD + beta_central_Rail_AMOD * central_dummy[i] + beta_shop * log(1+shop[i])*shop_stop_dummy+beta_work * log(1+employment[i])*work_stop_dummy + (d1[i]+d2[i]) * beta_distance_Rail_AMOD + beta_female_Rail_AMOD * female_dummy
	end
	
	--utility function for AMOD_Pool 1-1169
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_AMOD_Pool + first_bound * cost_AMOD_Pool[i]* beta_cost_drive1_2_first + second_bound * cost_AMOD_Pool[i]* beta_cost_drive1_2_second + first_bound * tt_AMOD_Pool[i] * beta_tt_SMS_Pool_first + second_bound * tt_AMOD_Pool[i] * beta_tt_AMOD_Pool_second + beta_central_AMOD_Pool * central_dummy[i] + beta_shop * log(1+shop[i])*shop_stop_dummy+beta_work * log(1+employment[i])*work_stop_dummy + (d1[i]+d2[i]) * beta_distance_AMOD_Pool + beta_female_AMOD_Pool * female_dummy
	end
	--utility function for Rail_AMOD_Pool 1-1169
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_Rail_AMOD_Pool + cost_Rail_AMOD_Pool[i] * beta_cost_Rail_AMOD_Pool_2 + tt_Rail_AMOD_Pool[i] * beta_tt_Rail_AMOD_Pool + beta_central_Rail_AMOD_Pool * central_dummy[i] + beta_shop * log(1+shop[i])*shop_stop_dummy+beta_work * log(1+employment[i])*work_stop_dummy + (d1[i]+d2[i]) * beta_distance_Rail_AMOD_Pool + beta_female_Rail_AMOD_Pool * female_dummy
	end
