...
local function computeUtilities(params,dbparams)
	...
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_AMOD + cost_AMOD[i] * beta_cost_drive1_1 + tt_AMOD[i] * beta_tt_AMOD + beta_central_AMOD * central_dummy[i] + beta_log * log(shop[i]+exp(beta_employment)*employment[i]) + (d1[i]+d2[i]) * beta_distance_AMOD + beta_female_AMOD * female_dummy
	end
	
	--utility function for Rail_AMOD 1-1169
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_Rail_AMOD + cost_Rail_AMOD[i] * beta_cost_Rail_AMOD_1 + tt_Rail_AMOD[i] * beta_tt_Rail_AMOD + beta_central_Rail_AMOD * central_dummy[i] + beta_log * log(shop[i]+exp(beta_employment)*employment[i]) + (d1[i]+d2[i]) * beta_distance_Rail_AMOD + beta_female_Rail_AMOD * female_dummy + beta_mode_work_bus * mode_work_Rail_AMOD
	end
		
	--utility function for AMOD_Pool 1-1169
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_AMOD_Pool + cost_AMOD_Pool[i] * beta_cost_drive1_1 + tt_AMOD_Pool[i] * beta_tt_AMOD_Pool + beta_central_AMOD_Pool * central_dummy[i] + beta_log * log(shop[i]+exp(beta_employment)*employment[i]) + (d1[i]+d2[i]) * beta_distance_AMOD_Pool + beta_female_AMOD_Pool * female_dummy
	end
	
	--utility function for Rail_AMOD_Pool 1-1169
	for i=1,1169 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_Rail_AMOD_Pool + cost_Rail_AMOD_Pool[i] * beta_cost_Rail_AMOD_Pool_1 + tt_Rail_SMS_Pool[i] * beta_tt_Rail_AMOD_Pool + beta_central_Rail_AMOD_Pool * central_dummy[i] + beta_log * log(shop[i]+exp(beta_employment)*employment[i]) + (d1[i]+d2[i]) * beta_distance_Rail_AMOD_Pool + beta_female_Rail_AMOD_Pool * female_dummy + beta_mode_work_bus * mode_work_Rail_AMOD_Pool
	end
end
