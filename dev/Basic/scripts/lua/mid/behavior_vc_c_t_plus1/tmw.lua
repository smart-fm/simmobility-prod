...
local function computeUtilities(params,dbparams)
	...
	local tt_bus_all=tt_bus_ivt+tt_bus_wait+tt_bus_walk + 1.0/60 -- Eytan added 1.0/60
	-- share of Bus travel time
	local tt_bus_ivt = tt_bus_ivt * (1.0/(60 * tt_bus_all - 1.0) + 1) -- Eytan
	local tt_bus_wait = tt_bus_wait * (1.0/(60 * tt_bus_all - 1.0) + 1) -- Eytan
	local tt_bus_walk = tt_bus_walk * (1.0/(60 * tt_bus_all - 1.0) + 1) -- Eytan
	...
	local tt_mrt_all=tt_mrt_ivt+tt_mrt_wait+tt_mrt_walk + 1.0/60 -- Eytan added 1.0/60
	-- Share of Rail travel time
	local tt_mrt_ivt = tt_mrt_ivt * (1.0/(60 * tt_bus_all - 1.0) + 1) -- Eytan
	local tt_mrt_wait = tt_mrt_wait * (1.0/(60 * tt_bus_all - 1.0) + 1) -- Eytan
	local tt_mrt_walk = tt_mrt_walk * (1.0/(60 * tt_bus_all - 1.0) + 1) -- Eytan
	...
	local tt_rail_SMS_all=tt_mrt_ivt+tt_mrt_wait+tt_mrt_walk + 1.0/60 -- Eytan added 1.0/60
	-- Share Shares of Rail-SMS travel time
	local tt_rail_SMS_ivt = tt_rail_SMS_ivt * (1.0/(60 * tt_rail_SMS_all - 1.0) + 1) -- Eytan
	local tt_rail_SMS_wait = tt_rail_SMS_wait * (1.0/(60 * tt_rail_SMS_all - 1.0) + 1) -- Eytan
	local tt_rail_SMS_walk = tt_rail_SMS_walk * (1.0/(60 * tt_rail_SMS_all - 1.0) + 1) -- Eytan
	...
	local tt_privatebus_all=tt_privatebus_ivt+tt_privatebus_wait+tt_privatebus_walk + 1.0/60 -- Eytan added 1.0/60
	-- Share Shares of PrivateBus travel time
	local tt_privatebus_ivt = tt_privatebus_ivt * (1.0/(60 * tt_privatebus_all - 1.0) + 1) -- Eytan
	local tt_privatebus_wait = tt_privatebus_wait * (1.0/(60 * tt_privatebus_all - 1.0) + 1) -- Eytan
	local tt_privatebus_walk = tt_privatebus_walk * (1.0/(60 * tt_privatebus_all - 1.0) + 1) -- Eytan

	local tt_cardriver_ivt=tt_ivt_car_first+tt_ivt_car_second
	local tt_cardriver_out=1.0/6
	local tt_cardriver_all=tt_cardriver_ivt+tt_cardriver_out + 1.0/60 -- Eytan added 1.0/60

	local tt_carpassenger_ivt=tt_ivt_car_first+tt_ivt_car_second
	local tt_carpassenger_out=1.0/6
	local tt_carpassenger_all=tt_carpassenger_ivt+tt_carpassenger_out + 1.0/60 -- Eytan added 1.0/60

	local tt_motor_ivt=tt_ivt_car_first+tt_ivt_car_second
	local tt_motor_out=1.0/6
	local tt_motor_all=tt_motor_ivt+tt_motor_out + 1.0/60 -- Eytan added 1.0/60

	local tt_walk=(d1+d2)/5 + 1.0/60 -- Eytan added 1.0/60

	local tt_taxi_ivt=tt_ivt_car_first+tt_ivt_car_second
	local tt_taxi_out=1.0/6
	local tt_taxi_all=tt_cardriver_ivt+tt_cardriver_out + 1.0/60 -- Eytan added 1.0/60

	local tt_SMS_ivt=tt_ivt_car_first+tt_ivt_car_second
	local tt_SMS_out=1/6.0
	local tt_SMS_all=tt_cardriver_ivt+tt_cardriver_out + 1.0/60 -- Eytan added 1.0/60
