			...
			for j in range(self.NUM_ZONES):
				...
				if (newCarIvt+newPubIvt+newPubWtt+newPubWalkt) > 0:
					...
					if newCarIvt > 0:
						...
						update_param_tuple = update_param_tuple + (newCarIvt, ALPHA, 1- ALPHA)
					...
					if newPubIvt > 0:
						...
						update_param_tuple = update_param_tuple + (newPubIvt,  ALPHA, 1-ALPHA)
					...
					if newPubWtt > 0:
						...
						update_param_tuple = update_param_tuple + (newPubWtt,  ALPHA, 1-ALPHA)
						comma_required = True
					if newPubWalkt > 0:
						if comma_required:
							update_query = update_query + ","
						update_query = update_query + " pub_walkt=(%s * %s+ " + AM_COSTS_TABLE + ".pub_walkt * %s)"
						update_param_tuple = update_param_tuple + (newPubWalkt,  ALPHA, 1-ALPHA)
				...
				for k in range(48):
					...
					if (newTTCarArr+newTTCarDep) > 0:
						...
						if newTTCarArr > 0:
							...
							update_param_tuple = update_param_tuple + (newTTCarArr,  ALPHA, 1-ALPHA)
							comma_required = True
						if newTTCarDep > 0:
							if comma_required:
								update_query = update_query + ","
							update_query = update_query + " " + departure_col + "=(%s * %s+ " + TCOST_CAR_TABLE + "." + departure_col + " * %s)"
							update_param_tuple = update_param_tuple + (newTTCarDep,  ALPHA, 1-ALPHA)
							comma_required = True
				if updatesAvailable:
					update_query = update_query + " WHERE origin_zone=%s and destination_zone=%s"
					update_param_tuple = update_param_tuple + (orgZ, desZ)
				...
				for k in range(48):
					...
					if (newTTBusArr+newTTBusDep) > 0:
						...
						if newTTBusArr > 0:
							...
							update_param_tuple = update_param_tuple + (newTTBusArr,  ALPHA, 1-ALPHA)
							comma_required = True
						if newTTBusDep > 0:
							if comma_required:
								update_query = update_query + ","
							update_query = update_query + " " + departure_col + "=(%s * %s+ " + TCOST_BUS_TABLE + "." + departure_col + " * %s)"
							update_param_tuple = update_param_tuple + (newTTBusDep,  ALPHA, 1-ALPHA)
							comma_required = True
				if updatesAvailable:
					update_query = update_query + " WHERE origin_zone=%s and destination_zone=%s"
					update_param_tuple = update_param_tuple + (orgZ, desZ)
