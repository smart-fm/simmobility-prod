...
sim_mob::BufferContainer &Broker::getReceiveBuffer()
{
	//		boost::unique_lock< boost::shared_mutex > lock(*NS3_Communicator_Mutex);
...
,enabled(true) //If a Broker is created, we assume it is enabled.
{
	...
	connection.reset(new ConnectionServer(clientRegistrationWaitingList, Broker_Client_Mutex,client_register));
...
void Broker::clientEntityAssociation(subscription subscription_, const boost::tuple<unsigned int,unsigned int,sim_mob::session_ptr >& availableClient)
{
	//filling temp data
	subscription_.clientID = availableClient.get<0>();//client id
	//sim_mob::session_ptr session_ = availableClient.second;

	//filling real data
	subscription_.handler.reset(
			new sim_mob::ConnectionHandler(
				availableClient.get<2>(), /*client type*/
				*this,
...
Entity::UpdateStatus Broker::update(timeslice now)
{
	...
	//Ensure that we have enough clients to process(in terms of client type (like ns3, android emulator, etc) and quantity(like enough number of android clients) ).
	if (!waitForClients()) {
