...
bool AndroidClientRegistration::handle(sim_mob::Broker& broker, sim_mob::ClientRegistrationRequest request)
{
	...
	if(freeAgent == 0)
	{
		...
		SIM_MOB_SERVICE srv;
		BOOST_FOREACH(srv, clientEntry.requiredServices)
		{
			switch(srv)
			{
			case SIMMOB_SRV_TIME:
				broker.getPublishers()[SIMMOB_SRV_TIME]->Suscribe(COMMEID_TIME, CALLBACK_HANDLER(TimeEventArgs, ClientHandler::OnTime) );
				break;
			case SIMMOB_SRV_LOCATION:
				broker.getPublishers()[SIMMOB_SRV_LOCATION]->Suscribe(COMMEID_TIME, CALLBACK_HANDLER(LocationEventArgs, ClientHandler::OnLocation) );
				break;
			}
