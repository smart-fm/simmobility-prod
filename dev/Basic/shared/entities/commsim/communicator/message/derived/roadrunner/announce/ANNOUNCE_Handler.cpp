...
//	}
	...
	try
	{
		...
		{
			WarnOut( "Client " <<  sender_id << " of type " <<  sender_type << " not found" << std::endl);
			return;
		}
	...
	catch(std::out_of_range e)
	{
		...
		WarnOut( "Client type" <<  sender_type << " not found" << std::endl);
		return;
	}
	...
	{
		WarnOut(  "Invalid agent record" << std::endl);
		return;
	}
