...
public:
  ...
  void handle_connect(const boost::system::error_code& e,boost::asio::ip::tcp::resolver::iterator endpoint_iterator)
  {
    if (!e)
    {
      ...
      // decode the data that is read from the underlying socket.
    	std::cout << "Connect done, reading now ...\n";
      connection_.async_read(stock__, boost::bind(&client::handle_read, this,boost::asio::placeholders::error));
  ...
  void handle_read(const boost::system::error_code& e)
  {
    if (!e)
    {
      ...
      connection_.async_read(stock__,
...
int receive(int argc, char* argv[])
{
  try
  {
    ...
    sim_mob::client client(io_service, argv[1], argv[2]);
    io_service.run();
  }
