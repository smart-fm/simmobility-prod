...
void sim_mob::Conflux::initCandidateAgents() {
	...
	for (currSegsOnUpLinksIt i = currSegsOnUpLinks.begin(); currSegsOnUpLinksIt != currSegsOnUpLinks.end(); i++) {
		lnk = (*i).first;
		do {
			rdSeg = *((*i).second);
		...
		} while (currSegsOnUpLinks[lnk] != upstreamSegmentsMap[lnk].end() && candidateAgents[rdSeg] == nullptr);
	}
}
