...
void SharedController::computeSchedules()
{
	...
	ControllerLog() << "Requests to be scheduled " << requestsToBeScheduledInitially << ", available drivers "
	                << availableDriversInitially << std::endl;
	...
	if (!validRequests.empty() && !availableDrivers.empty())
	{
		...
		std::vector<boost::graph_traits<boost::adjacency_list<boost::vecS, boost::vecS, boost::undirectedS>>::vertex_descriptor> mate(
				validRequests.size());
		...
		while (request1 != validRequests.end())
		{
			...
			while (request2 != validRequests.end())
			{
				...
				double tripTime1 = getTT(startNode1, startNode2, ttEstimateType) + getTT(startNode2, destinationNode1,
				                                                                         ttEstimateType);

				// We also compute the travel time that user 2 would experience
				double tripTime2 = getTT(startNode2, destinationNode1, ttEstimateType) + getTT(destinationNode1,
				                                                                               destinationNode2,
				                                                                               ttEstimateType);
				...
				{
					bestTrips[std::make_pair(request1Index, request2Index)] = std::make_pair(tripTime1 + tripTime2,
					                                                                         "o1o2d1d2");
				...
				//{ o2 o1 d2 d1
				tripTime1 = getTT(startNode1, destinationNode2, ttEstimateType) + getTT(destinationNode2,
				                                                                        destinationNode1,
				                                                                        ttEstimateType);

				tripTime2 = getTT(startNode2, startNode1, ttEstimateType) + getTT(startNode1, destinationNode2,
				                                                                  ttEstimateType);
				    ...
				    && (tripTime2 <= desiredTravelTimes.at(request2Index) + (*request2).extraTripTimeThreshold))
				{
					...
					{
						std::pair<double, std::string> currBestTrip = bestTrips[std::make_pair(request1Index,
						                                                                       request2Index)];

						if (tripTime1 + tripTime2 < currBestTrip.first)
						{
							bestTrips[std::make_pair(request1Index, request2Index)] = std::make_pair(
									tripTime1 + tripTime2, "o2o1d2d1");
						}
					}
					else
					{
				...
				//{ o1 o2 d2 d1
				tripTime1 = getTT(startNode1, startNode2, ttEstimateType) + getTT(startNode2, destinationNode2,
				                                                                  ttEstimateType) +
				...
				//{ o2 o1 d1 d2
				tripTime2 = getTT(startNode2, startNode1, ttEstimateType) + getTT(startNode1, destinationNode1,
				                                                                  ttEstimateType) +
		...
		ControllerLog() << "About to perform matching on " << requestQueue.size() << " requests and "
				...
				)
		{
			if (mate[*vi] != boost::graph_traits<boost::adjacency_list<boost::vecS, boost::vecS, boost::undirectedS>>::null_vertex() && *vi < mate[*vi])
			{
				...
				const std::pair<double, std::string> sharedTripInfo = bestTrips.at(
						std::make_pair(request1Idx, request2Idx));
	...
	ControllerLog() << "Schedules computed, the requests were " << requestsToBeScheduledInitially
	                << ", the drivers available were " <<
	                availableDriversInitially << ". Now the requests to be scheduled are " << requestQueue.size()
	                << ", available drivers " << availableDrivers.size();
	ControllerLog() << ". Performance profilingTime_directTripTime=" << toMs(profilingTime_directTripTime)
	                << ", profilingTime_graphConstruction=" <<
	                toMs(profilingTime_graphConstruction) << ", profilingTime_Matching=" << toMs(profilingTime_Matching)
	                << ", profilingTime_sendAssignement" <<
	                toMs(profilingTime_sendAssignement) << ", profilingTime_removeServedRequests="
	                << toMs(profilingTime_removeServedRequests) << std::endl;
}
...
void
SharedController::sendCruiseCommand(const Person *driver, const Node *nodeToCruiseTo, const timeslice currTick) const
{
	if (driverSchedules.find(driver) == driverSchedules.end())
	{
		std::stringstream msg;
		msg << "Trying to send a message to driver " << driver->getDatabaseId() << " pointer " << driver <<
