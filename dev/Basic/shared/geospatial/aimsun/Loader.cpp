...
//	bool LoadPathSetDBwithId(std::map<std::string,sim_mob::PathSet* >& pool,std::string& pathset_id);
	...
	static bool LoadOnePathSetDBwithIdST(soci::session& sql,std::string& pathset_id,sim_mob::PathSet& ps, const std::string tableName);
	void InsertPathSet2DB(std::map<std::string,sim_mob::PathSet* >& pathSetPool,const std::string pathSetTableName);
				...
				std::set<sim_mob::SinglePath*,sim_mob::SinglePath>& spPool)
{
	...
	//		std::cout<<"LoadSinglePathDB:  "<<i<<std::endl;
		...
		if (i==0)
		{
			sim_mob::Logger::instance["path_set"]  << "LSPDBwithId: "<<pathset_id<< "no data in db"<<std::endl;
			return false;
		}
		return true;
}
				...
				const std::set<const sim_mob::RoadSegment *> & excludedRS)
{
	...
	if(excludedRS.size()){
		...
		sql << excludeTable;
		sim_mob::Logger::instance["path_set"]  << excludeTable << std::endl;
		sql.commit();
...
//		sim_mob::Logger::instance["path_set"]  << selectQuery.str() << std::endl;
	}
	else{
...
//	if(rs.begin() == rs.end()){
//		sim_mob::Logger::instance["path_set"]  << "the above selectQuery has no results" << (excludedRS.size() ? "ex" : "") << std::endl;
//	}
	int i=0;
//	selectQuery.str("");
	for (soci::rowset<sim_mob::SinglePath>::const_iterator it=rs.begin(); it!=rs.end(); ++it)  {
...
//		sim_mob::Logger::instance["path_set"]  << r << " " ;
		r = spPool.insert(s).second;
//		sim_mob::Logger::instance["path_set"]  << r << std::endl;
		i++;
	}
//	sim_mob::Logger::instance["path_set"]  << selectQuery.str() << std::endl;
	if (i==0)
...
//		std::cout<<"LoadPathSetDB:  "<<i<<std::endl;
	...
	if(i==0)
	{
		sim_mob::Logger::instance["path_set"]  <<"LPSetDBwithId: ["<<query<<"] no data in db"<<std::endl;
		return false;
	}
	else
	{
...
void DatabaseLoader::InsertPathSet2DB(std::map<std::string,sim_mob::PathSet* >& pathSetPool,const std::string pathSetTableName)
{
