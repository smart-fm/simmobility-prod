...
std::vector<Agent const *> sim_mob::SimAuraManager::nearbyAgents(Point const &position, WayPoint const &wayPoint, double distanceInFront, double distanceBehind, 
																 const sim_mob::Agent* refAgent) const
{
	//Use the optimised bottom-up query, please read the paper to get more insights on "bottom-up query"
...
std::vector<Agent const *> sim_mob::SimAuraManager::nearbyAgentsBottomUpQuery(const Point &position, const WayPoint &wayPoint, double distanceInFront, 
																			  double distanceBehind, TreeItem *item) const
{
	// Find the stretch of the poly-line that <position> is in.
	std::vector<PolyPoint> points;
