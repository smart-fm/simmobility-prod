...
struct BigTableUpdate : std::unary_function<const Entity *, void>
{
	...
	{
		const Agent *agent = dynamic_cast<const Agent *>(item);
		
		if (agent && agent->xPos > 0 && agent->yPos > 0)
		{
			int x = (agent->xPos - SimRTree::network_minimum_x) / SimRTree::division_x_unit_;
			int y = (agent->yPos - SimRTree::network_minimum_y) / SimRTree::division_y_unit_;
 ...
 * Implement
 */
void SimRTree::buildTreeStructure(const std::string& filename)
{
...
void SimRTree::buildTreeStructure()
{
	//find the border
	int min_x = INT_MAX;
	int min_y = INT_MAX;
	...
	const map<unsigned int, Node *> &nodes = rdNetwork->getMapOfIdvsNodes();
		
	for (map<unsigned int, Node *>::const_iterator itr = nodes.begin(); itr != nodes.end(); ++itr)
	{
		double x = itr->second->getLocation().getX();
		double y = itr->second->getLocation().getY();
 ...
 *not implemented
 */
void SimRTree::releaseTreeMemory()
{
	releaseNodeMemory(m_root);
}

void SimRTree::releaseNodeMemory(TreeItem* item)
{
	if (!item->is_leaf)
	{
		TreeNode* node = static_cast<TreeNode*> (item);
 ...
 *
 */
void SimRTree::addNodeToFather(std::size_t father_id, TreeNode * from_node, TreeNode* new_node)
{
 ...
 *
 */
void SimRTree::addLeafToFather(std::size_t father_id, TreeNode * from_node, TreeLeaf* new_leaf)
{
	if (from_node->is_leaf)
		return;
 ...
 *
 */
SimRTree::BoundingBox SimRTree::locationBoundingBox(Agent *agent)
{
	SimRTree::BoundingBox box;
...
SimRTree::BoundingBox SimRTree::ODBoundingBox(Agent *agent)
{
	const Person *person = dynamic_cast<const Person *>(agent);	
	const Point &originLoc = person->originNode.node->getLocation();
...
void SimRTree::insertAgent(Agent* agent, std::map<const Agent*, TreeItem*>& connectorMap)
{
	SimRTree::BoundingBox agent_box = locationBoundingBox(agent);
	insertAgentEncloseBox(agent, agent_box, (m_root), connectorMap);
}

void SimRTree::insertAgentBasedOnOD(Agent* agent, std::map<const Agent*, TreeItem*>& connectorMap)
{
	SimRTree::BoundingBox agent_box = ODBoundingBox(agent);
	insertAgentEncloseBox(agent, agent_box, (m_root), connectorMap);
}

void SimRTree::insertAgentEncloseBox(Agent* agent, SimRTree::BoundingBox & agent_box, TreeItem* item, 
											  std::map<const Agent*, TreeItem*>& connectorMap)
{
...
std::vector<Agent const*> SimRTree::rangeQuery(SimRTree::BoundingBox & box) const
{
...
std::vector<Agent const*> SimRTree::rangeQuery(SimRTree::BoundingBox & box, TreeItem* item) const
{
	std::vector<Agent const*> result;
 ...
 *
 */
void SimRTree::updateAllInternalAgents(std::map<const Agent*, TreeItem*>& connectorMap, const std::set<Entity*>& removedAgentPointers)
{
	TreeLeaf* one_leaf = first_leaf;
	...
	while (one_leaf)
	{
		...
		while (offset < one_leaf->agent_buffer.size())
		{
			...
			{
				std::map<const Agent*, TreeItem*>::iterator it = connectorMap.find(one_agent);
...
void SimRTree::display()
{
	PrintOut("#########################################" << std::endl);
	display(m_root, 1);
}

void SimRTree::display(TreeItem* item, int level)
{
	if (!item->is_leaf)
	{
		PrintOut("Node level:" << level << "(" << item->bound.edges[0].first << "," << item->bound.edges[1].first << "," 
				<< item->bound.edges[0].second << "," << item->bound.edges[1].second << ")" << std::endl);
	...
	{
		TreeLeaf* one_leaf = static_cast<TreeLeaf*> (item);
		PrintOut("Leaf level:" << level << "(" << item->bound.edges[0].first << "," << item->bound.edges[1].first << "," 
				<< item->bound.edges[0].second << "," << item->bound.edges[1].second << ")" << "child size" << one_leaf->agent_buffer.size() << std::endl);
	}
}
 ...
 */
void SimRTree::initRebalanceSettings()
{
	std::string lineBuf;
 ...
 * Load Balance
 */
void SimRTree::measureUnbalance(int time_step, std::map<const Agent*, TreeItem*>& agent_connector_map)
{
...
void SimRTree::rebalance(std::map<const Agent*, TreeItem*>& agent_connector_map)
{
	releaseTreeMemory();
