...
public:
	 ...
	 * @return void .
	 */
	void settings(std::string ipadress, int port, int updateTime, std::string mapFile, int blockingTime)
	{
		this->ipAddress = ipadress;
		this->port = port;
		this->updateTime = updateTime;
		this->mapFile = mapFile;
		this->waitingSeconds = blockingTime;
	}
...
private:
	...
	std::vector<Person_ST*> allPersons;
	
	/**record dispatching drivers*/
	std::vector<Person_ST*> allDrivers;
	
	/**keep all children agents to communicate with it*/
	std::vector<Person_ST*> allChildren;
...
protected:
	...
	explicit FMOD_Controller(int id = -1, const MutexStrategy& mtxStrat = sim_mob::MtxStrat_Buffered) : Agent(mtxStrat, id), connectPoint(new FMOD_Client(ioService)), waitingSeconds(10)
	{
	}
	std::string getFOMD_Time(std::string ts);
private:
	/**
	 ...
	 */
	MessageList handleOfferMessage(timeslice now, const std::string& msg);
...
private:
	...
	/**when vehicle initialize and park, it will store to this structure*/
