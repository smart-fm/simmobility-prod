...
sim_mob::BusDriver::BusDriver(Person* parent, MutexStrategy mtxStrat) :
	...
	//From santhosh_max_PTTrip_merge
...
//		vehicle->setAcceleration(newFwdAcc * 100);
	...
	//NOTE: Santhosh's branch removed this; remove if needed.
					...
					if(person) {
						...
						if(bustrip && bustrip->itemType==TripChainItem::IT_BUSTRIP) {
							...
							if (busline) {
								...
								//From Santhosh's branch; enable if needed.
...
double sim_mob::BusDriver::passengerGenerationNew(Bus* bus)// new function to generate passengers and dwell time calculation
{
	double DTijk = 0.0;
...
vector<BufferedBase*> sim_mob::BusDriver::getSubscriptionParams() {
	...
	//std::cout << "busStopRealTimes_vec_bus.size() " << busStopRealTimes_vec_bus.size() << std::endl;
