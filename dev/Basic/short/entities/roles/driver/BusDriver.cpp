...
sim_mob::BusDriver::BusDriver(Person* parent, MutexStrategy mtxStrat) :
		Driver(parent, mtxStrat), nextStop(nullptr), waitAtStopMS(-1), lastTickDistanceToBusStop(
				-1), lastVisited_BusStop(mtxStrat, nullptr), lastVisited_BusStopSequenceNum(
				mtxStrat, 0), real_DepartureTime(mtxStrat, 0), real_ArrivalTime(
				mtxStrat, 0), DwellTime_ijk(mtxStrat, 0), busstop_sequence_no(
				mtxStrat, 0), first_busstop(true), last_busstop(false), no_passengers_boarding(
				0), no_passengers_alighting(0) {
	BUS_STOP_WAIT_PASSENGER_TIME_SEC = 2;
	dwellTime_record = 0;
	passengerCountOld_display_flag = false;
	curr_busStopRealTimes = new Shared<BusStop_RealTimes>(mtxStrat,
			BusStop_RealTimes());
...
Vehicle* sim_mob::BusDriver::initializePath_bus(bool allocateVehicle) {
	...
	if (!parent->getNextPathPlanned()) {
		...
		if (person) {
			...
			if (bustrip
					&& (*(person->currTripChainItem))->itemType
				...
				if ((*(person->currTripChainItem))->itemType
				...
				if ((*(person->currTripChainItem))->itemType
					...
					std::cout << "IT_BUSTRIP\n";
				std::cout
						<< "BusTrip path not initialized coz it is not a bustrip, (*(person->currTripChainItem))->itemType = "
						<< (*(person->currTripChainItem))->itemType
						<< std::endl;
			}
...
void sim_mob::BusDriver::frame_init(UpdateParams& p) {
	...
	if (newVeh) {
		...
		BusTrip* bustrip_change =
				dynamic_cast<BusTrip*>(*(person->currTripChainItem));
		vehicle = new Bus(nullRoute, newVeh,
				bustrip_change->getBusline()->getBusLineID());
		delete newVeh;
		...
		if (person) {
			if (person->getAgentSrc() == "BusController") {
				...
				if (bustrip && bustrip->itemType == TripChainItem::IT_BUSTRIP) {
					...
					if (busStops.empty()) {
						std::cout
								<< "Error: No BusStops assigned from BusTrips!!! "
								<< std::endl;
						// This case can be true, so use the BusStops found by Path instead
						busStops = findBusStopInPath(
								vehicle->getCompletePath());
					}
				}
			} else {
...
vector<const BusStop*> sim_mob::BusDriver::findBusStopInPath(
		const vector<const RoadSegment*>& path) const {
...
double sim_mob::BusDriver::linkDriving(DriverUpdateParams& p) {
	if ((params.now.ms() / 1000.0 - startTime > 10)
			&& vehicle->getDistanceMovedInSegment() > 2000
	...
	if (!vehicle->hasNextSegment(true)) {
		...
		p.nextLaneIndex = std::min<int>(p.currLaneIndex,
				vehicle->getNextSegment()->getLanes().size() - 1);
		if (vehicle->getNextSegment()->getLanes().at(p.nextLaneIndex)->is_pedestrian_lane()) {
			p.nextLaneIndex--;
			p.dis2stop = vehicle->getCurrPolylineLength()
					- vehicle->getDistanceMovedInSegment() + 1000;
		} else
...
//	vehicle->setAcceleration(acci);
	...
	if (abs(vehicle->getTurningDirection() != LCS_SAME) && newFwdAcc > 0
			&& vehicle->getVelocity() / 100 > 10) {
		newFwdAcc = 0;
	}
...
//		vehicle->setAcceleration(newFwdAcc * 100);
	...
	newLatVel = lcModel->executeLaneChanging(p,
			vehicle->getAllRestRoadSegmentsLength(), vehicle->length,
	...
	if (isBusApproachingBusStop()) {
		...
		p.nextLaneIndex =
				vehicle->getCurrSegment()->getLanes().back()->getLaneID();
		LANE_CHANGE_SIDE lcs =
				mitsim_lc_model->makeMandatoryLaneChangingDecision(p);
		vehicle->setTurningDirection(lcs);
		double newLatVel;
		newLatVel = mitsim_lc_model->executeLaneChanging(p,
				vehicle->getAllRestRoadSegmentsLength(), vehicle->length,
				vehicle->getTurningDirection());
	...
	if (wait && isBusGngtoBreakDown()) {
		...
		std::cout << "tick " << waitAtStopMS << " velocity "
				<< vehicle->getVelocity() << " " << vehicle->getAcceleration()
				<< std::endl;
...
//		if ((vehicle->getVelocity()/100) > 0)
		...
		if ((vehicle->getVelocity()/100 < 0.1)
			...
			if ((waitAtStopMS == p.elapsedSeconds) && bus) {
				std::cout << "real_ArrivalTime value: "
						<< real_ArrivalTime.get() << "  DwellTime_ijk: "
						<< DwellTime_ijk.get() << std::endl;
				...
				if (BusController::HasBusControllers()) {
					...
					if (person) {
						...
						if (bustrip
							...
							if (busline) {
								if ((busstop_sequence_no.get() == 2)
										|| (busstop_sequence_no.get() == 6)) { // only use holding control at selected time points
									double waitTime = 0;
									waitTime =
											BusController::TEMP_Get_Bc_1()->decisionCalculation(
													busline->getBusLineID(),
													bustrip->getBusTripRun_SequenceNum(),
													busstop_sequence_no.get(),
													real_ArrivalTime.get(),
													DwellTime_ijk.get(),
													busStopRealTimes_vec.at(
														busstop_sequence_no.get()),
													lastVisited_BusStop.get(),
													true);
								...
								BusTrip* bustrip_change =
										dynamic_cast<BusTrip*>(*(person->currTripChainItem));
								bustrip_change->lastVisitedStop_SequenceNumber =
										busstop_sequence_no.get();

							} else {
								std::cout
										<< "Busline is nullptr, something is wrong!!! "
										<< std::endl;
	...
	if (bus && passenger_dist) {
		...
		//if first busstop,only boarding
			...
			no_passengers_boarding = (config.percent_boarding * 0.01)
					* no_passengers_busstop;
			if (no_passengers_boarding
		...
		DTijk = dwellTimeCalculation(0, 0, 0, no_passengers_alighting,
				no_passengers_boarding, 0, 0, 0, bus->getPassengerCount());
		return DTijk;
	} else {
		throw std::runtime_error(
				"Passenger distributions have not been initialized yet.");
		return -1;
	}
}

double sim_mob::BusDriver::dwellTimeCalculation(int busline_i, int trip_k,
...
double sim_mob::BusDriver::getDistanceToBusStopOfSegment(
	...
	if (person) {
		if (person->getAgentSrc() == "BusController") {
			...
			if (bustrip
					&& (*(person->currTripChainItem))->itemType
							== TripChainItem::IT_BUSTRIP) {
				std::vector<const BusStop*> busStops_temp =
						bustrip->getBusRouteInfo().getBusStops();
				std::cout << "busStops_temp.size(): " << busStops_temp.size()
						<< std::endl;

				int size = busStops_temp.size();

				for (int i = 0; i < size; i++)
					busStopRealTimes_vec.push_back(
							new Shared<BusStop_RealTimes>(
									ConfigParams::GetInstance().mutexStategy,
									BusStop_RealTimes()));
...
void sim_mob::BusDriver::AlightingPassengers(Bus* bus) //for alighting passengers
	...
	if (bus->getPassengerCount() > 0) {
		...
		while (itr != bus->passengers_inside_bus.end()) {
			...
			Passenger* passenger =
					p ? dynamic_cast<Passenger*>(p->getRole()) : nullptr;
			if (!passenger) {
				continue;
			}
...
//	for (size_t i = 0; i < Agent::all_agents.size(); i++)//loop through all the agents to find passenger agents who are waiting at the approaching bus stop
		...
		if ((abs(
				(passenger->getXYPosition().getX() / 1000)
						- (xpos_approachingbusstop / 1000)) <= 2)
				and (abs(
						(passenger->getXYPosition().getY() / 1000)
								- (ypos_approachingbusstop / 1000)) <= 2)) {
			...
			if (passenger->isAtBusStop() == true) //if passenger agent is waiting at the approaching bus stop
				...
				if (passenger->PassengerBoardBus(bus, this, p, busStops, k)
						== true) //check if passenger wants to board the bus
						{
