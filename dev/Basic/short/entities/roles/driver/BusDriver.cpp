...
//				std::cout << "BusDriver::updatePositionOnLink: pich up passengers" << std::endl;
				...
				if(!BusController::all_busctrllers_.empty()) {
					BusController::all_busctrllers_[0]->receiveBusInformation(1, 0, 0, p.currTimeMS);
					DailyTime departureTime = BusController::all_busctrllers_[0]->decisionCalculation(1, 0, 0, p.currTimeMS,lastVisited_BusStopSequenceNum.get());// need to be changed, only calculate once(no need every time calculation)
					real_DepartureTime.set(departureTime.offsetMS_From(ConfigParams::GetInstance().simStartTime));// BusDriver set RealDeparture Time
				}
			}
		}
