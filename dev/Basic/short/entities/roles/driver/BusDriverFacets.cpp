...
BusDriverMovement::BusDriverMovement(Person* parentAgent) :
...
Vehicle* BusDriverMovement::initialiseBusPath(bool createVehicle)
{
	...
	if (parent && !parent->getNextPathPlanned())
	{
		...
		{
			Print() << "BusTrip path not initialised. Trip chain item type: " << (*(getParent()->currTripChainItem))->itemType
					<< std::endl;
		}
...
void BusDriverMovement::frame_tick_output()
{
    ...
    if (ConfigManager::GetInstance().CMakeConfig().OutputEnabled())
	{
		...
		output << "(\"BusDriver\"" << "," << params.now.frame() << "," << getParent()->getId()
				<< ",{" << "\"xPos\":\"" << parentBusDriver->getPositionX()
				<< "\",\"yPos\":\"" << parentBusDriver->getPositionY()
				<< "\",\"angle\":\"" << (360 - (baseAngle * 180 / M_PI))
				<< "\",\"length\":\"" << bus->getLengthCm()
				<< "\",\"width\":\"" << bus->getWidthCm()
