...
DriverMovement::DriverMovement(Person *parentAgent, Driver *parentDriver) :
MovementFacet(parentAgent), parentDriver(parentDriver), trafficSignal(NULL), targetLaneIndex(0), lcModel(NULL), cfModel(NULL), intModel(NULL),
...
void DriverMovement::frame_init()
{
	...
	if (vehicle)
	{
		...
		parent->getRole()->setResource(vehicle);
	}
	else
	{
...
TravelMetric& DriverMovement::startTravelTimeMetric()
{
	...
	const Node *startNode = parent->originNode.node;
...
TravelMetric& DriverMovement::finalizeTravelTimeMetric()
{
	...
	const Node *endNode = parent->destNode.node;
...
void DriverMovement::applyDrivingModels(DriverUpdateParams &params)
{
	...
	if (parent->amodId != "-1" || parentDriver->IsBusDriver())
	{
...
Vehicle* DriverMovement::initializePath(bool createVehicle)
{
	...
	if (!parent->getNextPathPlanned())
	{
		...
		parentDriver->destination = parent->destNode.node;
		...
		{
			path = PathSetManager::getInstance()->getPath(*(parent->currSubTrip), false, NULL);
		}
		else
		{
		...
		if (createVehicle)
		{
			...
			buildPath(path, parent->startLaneIndex, parent->startSegmentId);
		}
	}
...
void DriverMovement::updateNearbyAgents()
{
	...
	for (vector<const Agent *>::iterator it = nearbyAgentsList.begin(); it != nearbyAgentsList.end(); ++it)
	{
		...
		const Person *nearbyAgent = dynamic_cast<const Person *> (*it);
