...
void sim_mob::PassengerMovement::setParentBufferedData()
{
	...
	{
		parent->xPos.set(parentPassenger->busdriver.get()->currPos.x);
		parent->yPos.set(parentPassenger->busdriver.get()->currPos.y);
	}
}
...
bool sim_mob::PassengerMovement::PassengerAlightBus(Driver* driver)
{
	...
	if(busdriver) {
		...
		parent->xPos.set(driver->currPos.x);
		parent->yPos.set(driver->currPos.y);
		displayX = driver->currPos.x;
		displayY = driver->currPos.y;
	}
