...
//	std::cout << end_node->location.getX() << "," << end_node->location.getY() << std::endl;
	...
	{
		const sim_mob::MultiNode *one_multinode = dynamic_cast<const sim_mob::MultiNode *> (start_node);
		const std::set<sim_mob::RoadSegment*>& start_node_segments = one_multinode->getRoadSegments();

		std::set<sim_mob::RoadSegment*>::const_iterator it = start_node_segments.begin();
		for (; it != start_node_segments.end(); it++)
		{
			buffer_road_segments.push_back(const_cast<sim_mob::RoadSegment*> (*it));
		}
	...
	{
		const sim_mob::MultiNode *one_multinode = dynamic_cast<const sim_mob::MultiNode *> (end_node);
		const std::set<sim_mob::RoadSegment*>& start_node_segments = one_multinode->getRoadSegments();
		...
		const sim_mob::Point2D* two_far_point)
{
	...
	if (one_cross)
	{
		if (one_cross->nearLine.first.getX() != one_near_point->getX() || one_cross->nearLine.first.getY() != one_near_point->getY())
		{
			return false;
		}
		if (one_cross->nearLine.second.getX() != two_near_point->getX() || one_cross->nearLine.second.getY() != two_near_point->getY())
		{
			return false;
		}

		if (one_cross->farLine.first.getX() != one_far_point->getX() || one_cross->farLine.first.getY() != one_far_point->getY())
		{
			return false;
		}
		if (one_cross->farLine.second.getX() != two_far_point->getX() || one_cross->farLine.second.getY() != two_far_point->getY())
		{
			return false;
		}
