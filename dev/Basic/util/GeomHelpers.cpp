...
//add by xuyan
Point2D sim_mob::getMiddlePoint2D(sim_mob::Point2D* start_point, sim_mob::Point2D* end_point,
		double offset)
{
	double distance = sim_mob::dist(start_point->getX(), start_point->getY(), end_point->getX(),
			end_point->getY());
	double location_x = start_point->getX() + (offset * 100) / distance * (end_point->getX()
			- start_point->getX());
	double location_y = start_point->getY() + (offset * 100) / distance * (end_point->getY()
			- start_point->getY());
...
//add by xuyan
const sim_mob::Link* sim_mob::getLinkBetweenNodes(sim_mob::Point2D* start_point,
		sim_mob::Point2D* end_point)
{
	...
	{
		if ((*it)->getStart()->location->getX() == start_point->getX()
				&& (*it)->getStart()->location->getY() == start_point->getY()
				&& (*it)->getEnd()->location->getX() == end_point->getX()
		...
		sim_mob::Point2D* end_point)
{
	...
	if (dynamic_cast<sim_mob::UniNode *> (start_node))
	{
		...
		const std::vector<const sim_mob::RoadSegment*>& start_node_segments =
				one_uninode->getRoadSegments();
	...
	if (dynamic_cast<sim_mob::UniNode *> (end_node))
	{
		...
		const std::vector<const sim_mob::RoadSegment*>& end_node_segments =
				one_uninode->getRoadSegments();
	...
	for (; it != buffer_road_segments.end(); it++)
	{
		if ((*it)->getStart()->location->getX() == start_point->getX()
				&& (*it)->getStart()->location->getY() == start_point->getY())
			if ((*it)->getEnd()->location->getX() == end_point->getX()
					&& (*it)->getEnd()->location->getY() == end_point->getY())
			{
				return (*it);
			}
...
bool isRoadItemTheCrossing(const sim_mob::RoadItem* one_item, sim_mob::Point2D* one_near_point, sim_mob::Point2D* two_near_point,
		sim_mob::Point2D* one_far_point, sim_mob::Point2D* two_far_point)
{
...
//
//	std::cout << "Can not find the crossing" << std::endl;
	return 0;
}
