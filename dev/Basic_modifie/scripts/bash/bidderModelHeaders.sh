...
if [ $# -ne 2 ]; then	
	...
	if [ $1 == "expectations" ]; then
		...
		sed -i '1 i \bid_timestamp ,seller_id, bidder_id, unit_id, bidder wp (total-price), speculation (discount), asking_uprice (total-price), floor area (sqft), type_id, target_uprice (total-price), bid_value (total-price), bids_counter (daily), status(1 = ACCEPTED)' $2
		echo " "
	else
