...
def convert(rn :geo.formats.sumo.RoadNetwork) -> geo.formats.simmob.RoadNetwork:
  ...
  for jn in rn.junctions.values():
    newNode = geo.formats.simmob.Intersection(global_id.next(), 100*__trans(__mirror(jn.pos.x,minX,maxX), tempMinX), 100*__trans(jn.pos.y, tempMinY))
  ...
  for ed in rn.edges.values():
    ...
    for ln in reversed(ed.lanes):
      ...
      poly = [Point(100*__trans(__mirror(p1.x,minX,maxX),tempMinX), 100*__trans(p1.y,tempMinY)), \
              Point(100*__trans(__mirror(p2.x,minX,maxX),tempMinX), 100*__trans(p2.y,tempMinY))]
